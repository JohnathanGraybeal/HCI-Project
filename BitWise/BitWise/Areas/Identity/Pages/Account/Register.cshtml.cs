...
namespace BitWise.Areas.Identity.Pages.Account
{
    ...
    {
        private readonly SignInManager<BitWiseUser> _signInManager;
        private readonly UserManager<BitWiseUser> _userManager;
        private readonly IUserStore<BitWiseUser> _userStore;
        ...
        private BitWiseUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<BitWiseUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(BitWiseUser)}'. " +
                    $"Ensure that '{nameof(BitWiseUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<BitWiseUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<BitWiseUser>)_userStore;
        }
    }
}
